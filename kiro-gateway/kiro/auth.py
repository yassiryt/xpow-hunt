...
class KiroAuthManager:
    ...
    def _save_credentials_to_sqlite(self) -> None:
        ...
        try:
            ...
            # Merge into the EXISTING token blob: update only the rotating fields
            # (access/refresh/expires) and PRESERVE everything else the gateway
            # doesn't manage (notably `provider` and `profile_arn`). Previously this
            # wrote a minimal subset that DROPPED those fields, corrupting kiro-cli's
            # stored credential and forcing a re-login after every gateway refresh.
            def _merged_value_for(_key: str) -> str:
                existing = {}
                try:
                    _c = conn.cursor()
                    _c.execute("SELECT value FROM auth_kv WHERE key = ?", (_key,))
                    _r = _c.fetchone()
                    if _r and _r[0]:
                        _parsed = json.loads(_r[0])
                ...
                if self._expires_at:
                    existing["expires_at"] = self._expires_at.isoformat()
            ...
            if self._sqlite_token_key:
                cursor.execute(
                    ...
                    (_merged_value_for(self._sqlite_token_key), self._sqlite_token_key)
                )
    ...
    async def get_access_token(self) -> str:
        ...
        async with self._lock:
            # --- Account-follow mode ---------------------------------------
            # Always track whatever account kiro-cli is currently logged into.
            # kiro-cli persists AND refreshes its live token in SQLite, so by
            # re-reading it here (throttled) we: (a) automatically follow account
            # switches in kiro-cli, and (b) avoid doing our own network refresh,
            # which would fight kiro-cli over refresh-token rotation. If the
            # SQLite token is actually expired (kiro-cli not running), we fall
            # through to the normal reload+refresh path below.
            if self._sqlite_db and os.environ.get("KIRO_FOLLOW_CLI", "1") != "0":
                import time as _time
                interval = float(os.environ.get("KIRO_FOLLOW_CLI_INTERVAL", "5"))
                now_mono = _time.monotonic()
                if now_mono - getattr(self, "_follow_reload_ts", 0.0) >= interval:
                    self._follow_reload_ts = now_mono
                    try:
                        self._load_credentials_from_sqlite(self._sqlite_db)
                    except Exception as e:
                        logger.debug(f"[follow-cli] SQLite reload failed: {e}")
