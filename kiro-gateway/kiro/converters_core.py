...
def build_kiro_payload(
    ...
    # --- Maximum thinking effort (env-gated) ---------------------------------
    # Force reasoning effort and/or a thinking-token budget on the upstream
    # request. Wire shape matches the Kiro client: the request body carries
    # additionalModelRequestFields.output_config.{effort, task_budget}.
    #   KIRO_FORCE_EFFORT        e.g. "max" (enum: low|medium|high|xhigh|max)
    #   KIRO_THINK_BUDGET_TOKENS e.g. "60000" -> task_budget {type:tokens,total:N}
    # Pushing effort to "max" plus a large token budget yields the deepest
    # reasoning the model will do (well beyond the default).
    import os as _os
    _effort = _os.environ.get("KIRO_FORCE_EFFORT", "").strip()
    _budget = _os.environ.get("KIRO_THINK_BUDGET_TOKENS", "").strip()
    if _effort or _budget:
        # Only apply to models that support reasoning effort. Default: all opus
        # models + sonnet-4.6. Configurable via KIRO_EFFORT_MODELS (comma
        # substrings matched against the model id). Empty list => all models.
        _allow = [s.strip().lower() for s in _os.environ.get("KIRO_EFFORT_MODELS", "opus,sonnet-4.6").split(",") if s.strip()]
        _mid = (model_id or "").lower()
        if (not _allow) or any(a in _mid for a in _allow):
            _oc: Dict[str, Any] = {}
            if _effort:
                _oc["effort"] = _effort
            if _budget:
                try:
                    _oc["task_budget"] = {"type": "tokens", "total": int(_budget)}
                except ValueError:
                    pass
            if _oc:
                payload["additionalModelRequestFields"] = {"output_config": _oc}
                logger.info(f"[effort] {model_id} -> output_config={_oc}")
