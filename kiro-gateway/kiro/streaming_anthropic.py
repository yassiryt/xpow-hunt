...
async def stream_kiro_to_anthropic(
    ...
    # Input tokens: prefer the real Kiro-payload count (what the model actually
    # receives), passed from the route. Fall back to a proper message count.
    ...
    try:
        ...
        # Empty-completion guard: if the upstream produced NO content at all
        # (no text, no thinking, no tool calls), emit a minimal placeholder text
        # block so we never forward a truly empty assistant message. A genuinely
        # empty completion would be recorded by the client and poison the history
        # (every later turn then also comes back empty and can't recover).
        if (not tool_blocks
                and not (full_content or "").strip()
                and not (full_thinking_content or "").strip()
                and not suppress_empty_placeholder):
            _ph = "(no output)"
            _idx = current_block_index
            yield format_sse_event("content_block_start", {
                "type": "content_block_start",
                "index": _idx,
                "content_block": {"type": "text", "text": ""},
            ...
            yield format_sse_event("content_block_delta", {
                "type": "content_block_delta",
                "index": _idx,
                "delta": {"type": "text_delta", "text": _ph},
            })
            yield format_sse_event("content_block_stop", {
                "type": "content_block_stop",
                "index": _idx,
            ...
            current_block_index += 1
            full_content = _ph
            logger.warning("Empty-completion guard: upstream returned no content; emitted placeholder text block")

        # Calculate output tokens
        output_tokens = count_tokens(full_content + full_thinking_content)
        
        # Kiro reports its own context_usage_percentage. Log it next to our real
        # payload count for calibration/threshold observability, but do NOT clobber
        # the reported input_tokens when we have the real count (message_start
        # already advertised it). Only fall back to the percentage when no real
        # count was provided.
        if context_usage_percentage is not None:
            ...
            logger.info(
                f"[tokens] kiro context_usage={context_usage_percentage:.1f}% "
                f"-> kiro_prompt≈{prompt_tokens:,} | reported_input={input_tokens:,}"
            )
...
async def collect_anthropic_response(
    ...
    # Count input tokens (prefer real Kiro-payload count from the route)
    input_tokens = 0
    if real_input_tokens is not None:
        input_tokens = real_input_tokens
    elif request_messages:
    ...
    # Kiro's own context_usage_percentage — log for calibration; only use it when
    # we have no real payload count (don't clobber the accurate value).
    if result.context_usage_percentage is not None:
        ...
        logger.info(
            f"[tokens] kiro context_usage={result.context_usage_percentage:.1f}% "
            f"-> kiro_prompt≈{prompt_tokens:,} | reported_input={input_tokens:,}"
        )
        if real_input_tokens is None:
            input_tokens = prompt_tokens
